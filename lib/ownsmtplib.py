...
class SMTP:
    ...
    def _get_socket2(self, port, host, timeout, bindIP):
        # This makes it simpler for SMTP_SSL to use the SMTP connect code
        # and just alter the socket connection bit.
        if self.debuglevel > 0:
            print>>stderr, 'connect:', (host, port)
            print>>stderr, 'connect:', bindIP 
        return socket.create_connection((port, host), timeout, bindIP)
        #return socket.create_connection((port, host), timeout, ('43.243.209.171', 9001))
